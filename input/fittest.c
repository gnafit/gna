...
double mean2[100];
    ...
    TCanvas *mm0=new TCanvas("mm0","",800,600);
    ...
    //gr22 = new TGraph(layers,cc,mean1);
    //gr22->Draw("ap");
    //TCanvas *mm3=new TCanvas("mm3","",800,600);
    //gr23 = new TGraph(layers,cc,mean2);
    //gr23->Draw("ap");
    mm0->SaveAs("aa0.C");
    //mm->SaveAs("aa.C");
    //mm2->SaveAs("aa2.C");
...
double* fittest2(int nnj)
{
    ...
    RooRealVar mg2("mg2","mg2",1100,1000,1550) ;
    RooRealVar sg2("sg2","sg2",50.,10,80) ;
    RooGaussModel gauss2("gauss2","gauss2",sim_nPE,mg2,sg2) ;
    


    // low energy tail 
    RooRealVar tau1("tau1","tau1",40,0.0001,400) ;  // noLAB_th
    RooRealVar tau2("tau2","tau2",40,0.0001,400) ;  // noLAB_th
    RooDecay decay("decay","decay",sim_nPE,tau1,gauss,RooDecay::Flipped ) ;
    RooDecay decay2("decay2","decay2",sim_nPE,tau2,gauss2,RooDecay::Flipped ) ;
    
    RooRealVar bkgfrac("bkgfrac","fraction of background",0.2,0.,1.) ; 
    RooAddPdf model("model","g1a",RooArgList(decay2,decay),bkgfrac);
    ...
    model.plotOn(frame) ;
    //decay.plotOn(frame) ;
    gauss.plotOn(frame,LineColor(kMagenta)) ;
    gauss2.plotOn(frame,LineStyle(kDashed),LineColor(kRed)) ;
    ...
    int ia=0;
    rer[ia++]=bkgfrac->getVal()*mg2->getVal()+(1-bkgfrac->getVal())*mg->getVal();
    rer[ia++]=sqrt(pow(bkgfrac->getVal()*mg2->getErrorHi(),2)+pow((1-bkgfrac->getVal())*mg->getErrorHi(),2));
    rer[ia++]=sqrt(pow(bkgfrac->getVal()*mg2->getErrorLo(),2)+pow((1-bkgfrac->getVal())*mg->getErrorLo(),2));
    if(rer[1]>15)cout<<" eeeee "<<nnj<<endl;
    return rer;
