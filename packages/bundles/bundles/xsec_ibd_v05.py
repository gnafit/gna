...
class xsec_ibd_v05(TransformationBundle):
    """Inverse beta-decay cross section from Vogel&Beacom v05
    Changes since xsec_ibd_v05:
        - Add major index
    """
    ...
    def build(self):
        ...
        with entryContext(subgraph="IBD"):
            ...
            for idx in self.nidx_major:
                econv = R.EvisToEe(labels=idx.current_format('Ee|{autoindex}'))
                self.econvs+=econv,

                # register it's input and output
                self.set_input('ee', idx, econv.Ee.Evis, argument_number=0)
                self.set_output('ee', idx, econv.Ee.Ee)

                if order==0:
                    # in 0th order and 1d case
                    # create 1d cross section
                    # with self.namespace("ibd"):
                    ibd = R.IbdZeroOrder(*self._arguments)
                    self.ibds+=ibd,

                    # register Enu input and output
                    self.set_input('enu', idx, ibd.Enu.Ee, argument_number=0)
                    self.set_output('enu', idx, ibd.Enu.Enu)

                    # register cross section input and output
                    self.set_input('ibd_xsec', idx, ibd.xsec.Ee, argument_number=0)
                    self.set_output('ibd_xsec', idx, ibd.xsec.xsec)

                    #label cross section for the graph
                    ibd.xsec.setLabel(idx.current_format('{{IBD xsec (0)|{autoindex}}}'))
                elif order==1:
                    # in 1th order and 2d case
                    # create 2d cross section
                    # with self.namespace("ibd"):
                    ibd = R.IbdFirstOrder(*self._arguments)
                    self.ibds+=ibd,

                    # register Enu inputs and output
                    self.set_input('enu', idx, ibd.Enu.Ee, argument_number=0)
                    self.set_input('enu', idx, ibd.Enu.ctheta, argument_number=1)
                    self.set_output('enu', idx, ibd.Enu.Enu)

                    # register cross section inputs and output
                    self.set_input('ibd_xsec', idx, ibd.xsec.Enu, argument_number=0)
                    self.set_input('ibd_xsec', idx, ibd.xsec.ctheta, argument_number=1)
                    self.set_output('ibd_xsec', idx, ibd.xsec.xsec)
                    # label cross section
                    ibd.xsec.setLabel(idx.current_format('IBD xsec (1)|{autoindex}'))

                    # register jacobian inputs and output
                    self.set_input('jacobian', idx, ibd.jacobian.Enu, argument_number=0)
                    self.set_input('jacobian', idx, ibd.jacobian.Ee, argument_number=1)
                    self.set_input('jacobian', idx, ibd.jacobian.ctheta, argument_number=2)
                    self.set_output('jacobian', idx, ibd.jacobian.jacobian)
                    # label jacobian
                    ibd.jacobian.setLabel(idx.current_format('Ee-\\>Enu jacobian|{autoindex}'))

                # label neutrino energy caclulator
                ibd.Enu.setLabel(idx.current_format('Enu|{autoindex}'))
