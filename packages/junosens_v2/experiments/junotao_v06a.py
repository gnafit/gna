...
class exp(baseexp):
    """\
JUNO+TAO experiment implementation v06a
To be presented on Monday 2021-12-06

Changes since junotao_v05d:
    + JUNO and TAO
        - Switch to the EnergyResolutionErfInput
    + TAO
        - Add TAO for NMO analysis
        - Update fast neutrons spectrum via A * exp(B*x) + C
        - Add TS2 to TAO neutrino flux
        - Add variance of TAO
        - Update Li9/Accidentals spectra for TAO
        # - General
        #     * Synchronize TAO binning with JUNO
        #     * TBD: sync threshold
        - Add new TAO inputs numbers:
            * target mass (number of protons), efficiency
            * duty cycle
            * TODO: veto_livetime
            * TODO: TAO duty cycle to be applied to backgrounds
        - Update detector effects:
            * New energy resolution: read from a root file and pre-interpolate
            * New energy leakage: read from a root file and pad
    + JUNO: consistent

Delete in the next version junotao_v06b:
    - [ ] Previous energy resolution
    - [ ] Previous energy leakage
    - [ ] Previous TAO input file
...
TODO:
    - Add variance_tao
        * Add fastn
    - Revise shape nuisance for TAO (M. Gonchar)
    - Binning of TAO (now it differs with fastn unc)
    - Add fastn_shape_unc_rel
    - Refactor formulas for TAO oscillation
    - Make lsnl_energy_scale parameters' state=free?
"""
    ...
    def initparser(cls, parser, namespace):
        ...
        consistency.add_argument("--expose-matrices", action="store_true", help="Expose LSNL/eres sparse matrices")

        consistency.add_argument("--old", nargs="+", default=(), choices=("bkg", "inputs", "eres", "eleak", "eresimpl"), help='compatibility mode (with previous version)')
        # consistency.add_argument("--old", nargs="+", default=(), choices=("bkg", "inputs", "eres", "eleak", "taobinning"), help='compatibility mode (with previous version)')
        ...
        spectrum.add_argument("--free-pars-mode", choices=["log", "plain"], default=None, help="type of the spectral uncertainty")
        spectrum.add_argument("--spectra-unc-hm", action="store_true", help="enable HM spectral uncertainties")
        ...
        parameters.add_argument("--bin2bin", default="nuisance", choices=("covmat", "nuisance"), help="bin2bin uncertainties propagation")

        # TAO
        parameters = parser.add_argument_group("tao-parameters", description="Parmeters setting for TAO")
        parameters.add_argument("--disable-ts2", action="store_true", help="Disable TS2 reactor from TAO prediction")
        parameters.add_argument("--oscprob-tao", default="vacuum", choices=("off", "vacuum", "sterile"), help="oscillation probability type for TAO")

        # Miscellaneous
        collapse=parser.add_mutually_exclusive_group()
        collapse.add_argument("--collapse", action="store_true", help="collapsed configuration")
        collapse.add_argument("--collapse2", action="store_true", help="collapsed configuration (2 reactors)")
    ...
    def init_reactors(self):
        ...
        self.reactors_lbl = [
            "FANGCHENGGANG-1", "FANGCHENGGANG-2", "CHANGJIANG-1", "CHANGJIANG-2", "FUQING-1", "FUQING-2", "FUQING-3", "FUQING-4",
            "NINGDE-1", "NINGDE-2", "NINGDE-3", "NINGDE-4", "SANMEN-1", "SANMEN-2", "FANGJIASHAN-1", "FANGJIASHAN-2",
            "QIANSHAN2-1", "QIANSHAN2-2", "QIANSHAN2-3", "QIANSHAN2-4", "QIANSHAN3-1", "QIANSHAN3-2", "QINSHAN-1",
            "TIANWAN-1", "TIANWAN-2", "TIANWAN-3", "TIANWAN-4", "HAIYANG-1",
        ]
        ...
        if self.opts.collapse or self.opts.collapse2:
            ts1idx = self.reactors_mbl.index("TS1")
            ts2idx = self.reactors_mbl.index("TS2")

            if self.opts.collapse:
                self.reactors_mbl = [self.reactors_mbl[ts1idx]]
            else:
                self.reactors_mbl = [self.reactors_mbl[ts1idx], self.reactors_mbl[ts2idx]]
    ...
    def init_nidx(self):
        ...
        self.nidx = [
            ...
            ("rt", "reactors_tao", ["TS1"] if self.opts.disable_ts2 else ["TS1", "TS2"]),
    ...
    def init_formula(self):
        ...
        formula_options = dict(
            ...
            oscprob_tao={
                "vacuum": "* sum[c]| pmns[c]*oscprob_tao[c,rt](enu_matrix)",
                "off": "",
            }[self.opts.oscprob_tao],
            #
            # Geo neutrino
            #
        ...
        old_inputs = 'inputs' in self.opts.old
        old_eres = 'eres' in self.opts.old
        self.formula = [
            ...
            "livetime_tao=bracket(daq_years_tao*seconds_in_year)" if old_inputs else "",
            "livetime_tao=bracket(duty_cycle_tao*veto_livetime_fraction_tao*daq_years_tao*seconds_in_year)" if not old_inputs else "",
            "conversion_factor",
            "efflivetime = bracket(eff * livetime)",
            "efflivetime_tao = bracket(eff_tao * livetime_tao * duty_cycle_tao)" if     old_inputs else "",
            "efflivetime_tao = bracket(eff_tao * livetime_tao)"                  if not old_inputs else "",
            ...
            "lsnl_coarse = lsnl_juno_escale * sum[l]| lsnl_weight[l] * lsnl_component_y[l]()",
            "lsnl_gradient = lsnl_juno_escale * sum[l]| lsnl_weight[l] * lsnl_component_grad[l]()",
            ...
            "baseline_tao",
            # TAO matrix
            "lsnl_coarse_tao = lsnl_tao_escale * sum[l]| lsnl_weight[l] * lsnl_component_y_tao[l]()",
            ...
            "eres_sigmarel_input| sigma_correction_fine()*eres_sigmarel| binning_smear_centers()",
            "eres_matrix| evis_hist_quench" if keeperes else "",
            # Eres TAO
            "eres_sigmarel_tao_fine_base| eres_tao_e_coarse(), binning_smear_noquench_centers(evis_hist_noquench)" if not old_eres else "",
            "eres_sigmarel_tao_fine| eres_tao_sigmarel_coarse()"                                                   if not old_eres else "",
            "eres_sigmarel_input_tao| eres_sigmarel_tao_fine()"                                                    if not old_eres else "",
            #
            # Neutrino energy
            #
            ...
            "enu_matrix = select1[m,'matrix']| enu()",
            #
            # Oscillations
            #
            "pmns[c]",
            ...
            "offeq_in_reac = offeq_scale * offeq_scale_individual[r] * nominal_spec_per_reac() * offeq_fine()",
            #
            # Backgrounds in JUNO
            #
            ...
            "reactors_elbl_num  = days_in_second * livetime * reactors_elbl_rate * reactors_elbl_rate_norm",
            #
            ...
            "bkg_shape_variance = bin_width_factor * bkgbin_widths(accidentals) * sumsq_bkg| lihe_bin2bin*lihe, fastn_bin2bin*fastn, alphan_bin2bin*alphan, geonu_bin2bin*geonu, reactors_elbl_bin2bin*reactors_elbl",
            #
            # Backgrounds in TAO
            #
            ...
            # "fastn_sigma_tao  = bracket| fastn_num_tao *                             rebin_fastn_sigma_tao| fastn_spectrum_tao()*rebin_fastn_shape_unc_rel|fastn_shape_unc_rel()",
            ...
            # IBD part
            #
            """anuspec_rd_full =  reactor_active_norm*( sum[i]| power_livetime_factor*anuspec_unc_corrected )
                                + reactor_active_norm*offeq_in_reac
                                + snf_in_reac
                                """,
            ...
            """ibd_juno_all=bracket| bracket(norm_reac*norm_juno)*anue_spectrum_scale_final()* rebin|
                         {eres} {lsnlmatrix}
                           rebin_juno_internal |
                               kinint2_juno(
                                   DistortSpectrum(
            ...
            # Total observation
            #
            "observation_juno=bracket| ibd_juno_all + bkg_juno",
            "unc_juno_anue = anue_bin2bin_unc_rel()*ibd_juno_all",
            "variance_juno_stat = ibd_juno_all",  # backgrounds disabled in stat-only mode
            "variance_juno_ibd  = ibd_juno_all     + unc_juno_anue*unc_juno_anue",  # backgrounds disabled in stat-only mode
            "variance_juno_all  = observation_juno + unc_juno_anue*unc_juno_anue + bkg_shape_variance",
            "variance_juno_bkg  = observation_juno +                               bkg_shape_variance",
            #
            # TAO part
            #
            "eres_matrix_tao| evis_hist_noquench",
            'anuspec_rd_full_matrix = select1[m,"matrix"]| anuspec_rd_full',
            'anuspec_rd_full_matrix_ts1=select1[r,"TS1"](anuspec_rd_full_matrix)',
            'anuspec_rd_full_matrix_ts2=select1[r,"TS2"](anuspec_rd_full_matrix)',
            'anuspec_rd_full_tao = selector_tao[rt](anuspec_rd_full_matrix_ts1)' if self.opts.disable_ts2 else
            'anuspec_rd_full_tao = selector_tao[rt](anuspec_rd_full_matrix_ts1, anuspec_rd_full_matrix_ts2)',
            'SpectralDistortion_tao = select1[m,"matrix"]| SpectralDistortion',
            """ibd_tao = bracket| norm_reac*norm_tao*rebin_tao| eres_tao|
                               {lsnl_tao}
                                 eleak_tao|
                                   rebin_tao_internal|
                                       kinint2_tao|
            ...
            # "bkg_shape_variance_tao = bin_width_factor * bkgbin_tao_widths(accidentals_tao) * sumsq_bkg_tao| lihe_bin2bin*lihe_tao, ff_bin2bin_tao*ibd_tao, fastn_bin2bin_tao*fastn_sigma_tao",
            # "variance_tao = observation_tao + bkg_shape_variance_tao",
            #
            # Comparison cases
            #
            ...
            #
            # TAO comparison cases
            #
            "cmp_rebin_tao_raw| rebin_tao_internal()" if not self.opts.old else "",
            "cmp_rebin_tao_eleak| eleak_tao()" if not self.opts.old else "",
            "cmp_rebin_tao_lsnl| lsnl_tao()" if not self.opts.old else "",
            "cmp_rebin_tao_eres| eres_tao()" if not self.opts.old else "",
            # TAO backgrounds
            "cmp_tao_accidentals = acc_num_tao    * acc_spectrum_tao()" if not self.opts.old else "",
            "cmp_tao_fastn       = fastn_num_tao  * fastn_spectrum_tao()" if not self.opts.old else "",
            "cmp_tao_lihe        = lihe_num_tao   * lihe_spectrum_tao()" if not self.opts.old else "",
            # "cmp_rebin_tao_bkg| cmp_tao_accidentals + cmp_tao_fastn + cmp_tao_lihe",
            #
            # Data
            #
            "rebin_data_juno",
    ...
    def init_configuration(self):
        ...
        if self.opts.fine_step:
            step = self.opts.fine_step * 1.0e-3

        if "eleak" in self.opts.old:
            step_tao = 0.01
        else:
            step_tao = 0.005
        edges_internal = np.arange(0.0, 12.001, step)
        edges_internal_tao = np.arange(0.0, 12.001, step_tao)
        juno_edges = np.concatenate(([0.8], np.arange(0.94, 7.44, 0.02), np.arange(7.44, 7.8, 0.04), np.arange(7.8, 8.2, 0.1), [8.2, 12.0]))
        ...
        anue_spec_unc_edges = np.arange(anue_spec_unc_edges[0], anue_spec_unc_edges[-1] + 1.0e-6, 0.01)
        ...
        if self.opts.comparison:
            ...
            a, b, c = (
                anue_spec_unc_edges[anue_spec_unc_edges < 1.99],
                anue_spec_unc_edges[(anue_spec_unc_edges >= 1.99) * (anue_spec_unc_edges < 8.01)],
                anue_spec_unc_edges[anue_spec_unc_edges >= 8.01],
            )
        ...
        # if not "taobinning" in self.opts.old:
        #     tao_edges = juno_edges.copy()
        #     tao_edges[0] = 0.9

        if self.opts.verbose:
            print()
            print("Binning (compressed):")
            ...
            if self.opts.verbose>2:
                print("Binning:")
                print("JUNO binning:", juno_edges)
                print("TAO binning:", tao_edges)
                print("Enu binning:", enu_edges)
                print("Enu spec. unc. binning", anue_spec_unc_edges)
                print()
        ...
        common_input_root_file_tao = "TAOInputs2021_11_29.root"

        bin2bin_nuisance_fixed = self.opts.bin2bin == "covmat"
        self.cfg = NestedDict(
            #
            # Numbers
            #
            ...
            bkg_shape_unc=dict(
                ...
                hooks={"bin_width_factor": lambda pars: (1.0 / pars["bin_width"], "1/`bin width`")},
                state="fixed",
            ),
            bkg_shape_unc_tao=dict(
                bundle=dict(name="parameters", version="v06"),
                pars=f"{common_input_location}/files/bkg_bin2bin_tao.yaml",
                state="fixed",
            ),
            tao_numbers=dict(
                bundle=dict(name="parameters", version="v06"),
                pars=f"{common_input_location}/files/tao_input_numbers_v2.py",
                skip=("percent",),
                state="fixed"
            ),
            baselines_tao=dict(
                bundle=dict(name="reactor_baselines", version="v03", major=("rt", ""), names=lambda s: s + "_tao"),
                reactors=f"{common_input_location}/files/reactor_baselines_tao_v2.yaml",
            ...
            baselines=dict(
                ...
                slices={"mbl": self.reactors_mbl.copy(), "lbl": "rest"},
                bundles=dict(
                    mbl=dict(
            ...
            reactor_spectrum_shape_unc=dict(
                ...
                label="JUNO bin2bin scale correction",
                errname="anue_bin2bin_unc_rel",
                errlabel="JUNO bin2bin relative uncertainty",
                mode="relative",
            ...
            bkg_rate_tao=dict(
                ...
                pars=f"{common_input_location}/files/bkg_rates_tao_v2.yaml",
                separate_uncertainty="{}_norm"
            ),
            #
            # Transformations
            ...
            energies=dict(
                bundle=dict(name="arrange", version="v01", major="m", minor=""),
                names="evis_all",
            ),
            selector=dict(
                bundle=dict(name="arrange", version="v01", major=("rt",), minor="", names="_tao"),
                names="selector",
            ),
            ...
            rebin_internal_tao=dict(
                ...
                instances={"rebin_tao_internal": "TAO internal binning"},
            ),
            rebin=dict(
            ...
            rebin_data_juno=dict(
                ...
                rounding=5,
                edges=juno_edges,
                instances={"rebin_data_juno": "JUNO data"}
            ),
            rebin_bkg=dict(
                bundle=dict(name="rebin", version="v04", major=""),
                rounding=5,
            ...
            rebin_bkg_tao=dict(
                ...
                instances={
                    ...
                    "rebin_fastn_shape_unc_rel": "Fastn neutrons corr. TAO",
                },
            ),
            rebin_tao=dict(
                bundle=dict(name="rebin", version="v04", major=""),
            ...
            rebin_comparison_bkg=dict(
                bundle=dict(name="rebin", version="v04", major=""),
                rounding=5,
                edges=np.arange(0.8, 12.0 + 1.0e-6, 0.02),
                instances={
                    "cmp_rebin_bkg": "Rebin comp. bkg.",
                    "cmp_rebin_tao_bkg": "Rebin bkg. TAO",
                    "cmp_rebin_tao_fastn": "Rebin fastneutrons TAO",
                },
            ),
            ...
            rebin_comparison5=dict(
                bundle=dict(name="rebin", version="v04", major=""),
                rounding=5,
                edges=np.arange(0.8, 12.0 + 1.0e-6, 0.005),
                instances={
                    "cmp_rebin_3": "Rebin comp. 3",
                    "cmp_rebin_12": "Rebin comp. 1&2",
                },
            ),
            rebin_comparison5_tao=dict(
                bundle=dict(name="rebin", version="v04", major=""),
                rounding=5,
                edges=np.arange(0.8, 12.0 + 1.0e-6, 0.005),
                instances={
                    "cmp_rebin_tao_raw": "Rebin comparison TAO raw",
                    "cmp_rebin_tao_eleak": "Rebin comparison TAO eleak",
                    "cmp_rebin_tao_lsnl": "Rebin comparison TAO lsnl",
                    "cmp_rebin_tao_eres": "Rebin comparison TAO eres",
                },
            ),
            #
            # Background shape variance
            #
            ...
            bkg_edges_tao=dict(
                bundle=dict(name="trans_histedges", version="v01", major=""),
                types=("widths",),
                instances={"bkgbin_tao": "Bkg bins TAO"}
            ),
            variance=dict(
                bundle=dict(name="trans_snapshot", version="v01", major=""),
                instances={
                    "sumsq_snapshot": "Bkg shape variance snapshot, not corrected|{autoindex}",
            ...
            variance_tao=dict(
                bundle=dict(name="trans_snapshot", version="v01", major=""),
                instances={
                    "sumsq_snapshot_tao": "Bkg shape variance snapshot, not corrected|TAO",
            ...
            eleak_tao=dict(
                ...
                filename        = f"{common_input_location}/{common_input_root_file_tao}",
                matrixname      = "TAOEnergyLeakageMatrix",
                pad             = (160, 0),
                matrix_is_upper = False,
            ),
            eres_sigmarel_tao=dict(
                bundle          = dict(name              =    "root_graphs", version    =    "v01"),
                filename        = f"{common_input_location}/{common_input_root_file_tao}",
                formats         = ["TAOEnergyResolutionNoNeutronRecoil"],
                names           = [("eres_tao_e_coarse", "eres_tao_sigmarel_coarse")],
                labels          = [("TAO eres E (coarse)", "TAO eres sigma rel (coarse)")],
                preinterpolate  = dict(npoints=20, interp_kwargs=dict(kind='pchip')),
                # debug_show      = True
            ),
            eres_sigmarel_tao_fine=dict(
                bundle=dict(name="interpolation_1d", version="v01", major=""),
                name="eres_sigmarel_tao_fine",
                kind="linear",
                strategy=("extrapolate", "extrapolate"),
                label="TAO Eres idx|{autoindex}",
                labelfmt="TAO Eres sigma rel fine|{autoindex}",
            ),
            eres_smearing_tao=dict(
                bundle=dict(name="detector_eres_inputsigma", version="v02", major="", names=lambda s: s + "_tao"),
                expose_matrix=self.opts.expose_matrices or False,
                mode='erf'
            ),
            #
            # IBD
            ...
            oscprob_matter=dict(
                bundle=dict(name="oscprob_matter", version="v02", major="rd", inactive=self.opts.oscprob == "vacuum", names=dict(oscprob="oscprob_matter")),
                density=2.45,  # g/cm3
            ),
            oscprob_tao=dict(
                bundle=dict(name="oscprob_ee", version="v01_half", major=("rt", "c"), names=dict(baseline="baseline_tao", pmns="pmns", oscprob="oscprob_tao"),),
                reactors=["TS1"] if self.opts.disable_ts2 else ["TS1", "TS2"],
            ),
            #
            ...
            numbers_wc=dict(
                ...
                labels={"baseline_wc": "Baseline for worst case distortion, km"},
            ...
            oscprob_wc_no=dict(
                bundle=dict(
                    name="oscprob_ee", version="v01", major=("", "", "c"),
            ...
            condproduct=dict(
                bundle=dict(name="conditional_product", version="v01", major=(), names={"condition": "distortion_wc_on"}),
            ...
            condproduct_tao=dict(
                bundle=dict(name="conditional_product", version="v01", major=(), names={"condition": "distortion_wc_on"}),
                instances={"DistortSpectrumTAO": "{{Optionally distorted spectrum [TAO] | worst case}}"},
                condlabel="Worst case distortion switch",
                default=0,
            ),
            #
            ...
            anuspec_hm_unc=dict(
                ...
                ename="enu_unc",
                filename=[
                    "data/data_juno/anue_spectra/hm_interpolation/10_keV_bin/Huber_{type}rrelated_unc_extrap_{isotope}_13.0_0.01_MeV.txt",
            ...
            offeq_scale=dict(
                bundle=dict(name="parameters", version="v06"),
                labels={"offeq_scale": "Offeq. scale"},
                pars={
                    "offeq_scale": (1.0, 30),
                    "uncertainty_mode": "percent",
                },
            ),
            offeq_scale_individual=dict(
                bundle=dict(name="parameters", version="v06", major=()),
                labels={"offeq_scale_individual": "Offeq. scale (individual)"},
                pars={
                    "offeq_scale_individual": 1.0,
            ...
            bump_correction_coarse_centers=dict(
                bundle=dict(name="trans_histedges", version="v01"), types=("centers",), instances={"bump_correction_coarse": "Bump correction centers"}
            ),
            bump_correction_in=dict(
                bundle=dict(name="root_histograms", version="v05"),
                filename=f"{common_input_location}/{common_input_root_file}",
            ...
            bump_switch=dict(
                ...
                instances={"bump_switch": "{{n=0.95 or bump|{autoindex}}}"},
            ...
            fission_fractions=dict(
                bundle=dict(name="parameters_yaml", version="v01", major="i"),
            ...
            lsnl=dict(
                bundle=dict(
                    name="energy_nonlinearity_db_root_subst",
                    version="v02",
                    major="l",
                ),
                names={
            ...
            lsnl_tao=dict(
                ...
                names={
                    ...
                    "pull2": "positronScintNLpull2",
                    "pull3": "positronScintNLpull3",
                    # "pull4": "positronScintNLpull4",
                },
                filename=f"{common_input_location}/{common_input_root_file}",
                nonlin_range=(0.499, 12.0),
                extrapolate_range=(1.0, 12.0),
                supersample=20,
                expose_matrix=False,
                create_weights=False,
            ),
            lsnl_energy_scale=dict(
                bundle=dict(name="parameters", version="v05"),
                state="fixed",
                labels=dict(
                    lsnl_juno_escale = "Extra energy scale factor for JUNO",
                    lsnl_tao_escale = "Extra energy scale factor for TAO",
                ),
                pars=dict(
                    lsnl_juno_escale = (1.0, 0.2),
                    lsnl_tao_escale  = (1.0, 0.2),
                    uncertainty_mode = 'percent',
                ),
            ),
            ...
            bins_internal=dict(
                ...
                instances={
                    ...
                    'binning_smear_noquench': 'E centers (smearing) [noquench]',
            ...
            bkg_spectra=dict(
                ...
                labels=[
                    "Acc. JUNO|(norm spectrum)", "9Li/8He JUNO|(norm spectrum)", "Fast n JUNO|(norm spectrum)",
                    "AlphaN JUNO|(norm spectrum)", "GeoNu Th232 JUNO|(norm spectrum)", "GeoNu U238 JUNO|(norm spectrum)",
                ],
                normalize=True,
            ),
            ...
            bkg_spectra_tao=dict(
                bundle=dict(name="root_histograms", version="v05"),
                filename=f"{common_input_location}/{common_input_root_file_tao}",
                formats=["TAOAccBkg", "TAOLi9Bkg"],
                names=["acc_spectrum_tao", "lihe_spectrum_tao"],
                labels=["Accidentals TAO|(norm spectrum)", "9Li/8He TAO|(norm spectrum)"],
                normalize=True,
            ),
            bkg_spectra_fastn_tao=dict(
                bundle=dict(name="tao_fastn_spectrum", version="v01"),
                name="fastn_spectrum_tao",
                parameters={
                    "A": (136./5, 0.),
                    "B": (-1., 0.),
                    "C": (10./5, 0.),
                },
                label="Fast n TAO|(norm spectrum)",
                order=4,
                edges=edges_internal_tao,
                normalize=True,
            ),
            bkg_spectra_unc_tao=dict(
                ...
                labels=["Fast n shape unc 3 years|(relative)"],
                normalize=True,
            ),
            ...
            geonu_fractions=dict(
                bundle=dict(name="var_fractions", version="v02"),
                names=["Th232", "U238"],
                format="frac_{component}",
                fractions=uncertaindict(Th232=(0.23, "fixed")),
            ),
            #
            ...
            bkg_shape_fastn=dict(
                ...
                uncertainty={
                    ...
                    "uncertainty": "fastn_bin2bin", "binwidth": "bin_width"
                },
                fixed=bin2bin_nuisance_fixed,
            ),
            bkg_shape_lihe=dict(
                ...
                uncertainty={
                    "yaml": f"{common_input_location}/files/bkg_bin2bin_v2.yaml",
                    "uncertainty": "lihe_bin2bin", "binwidth": "bin_width"
                },
                fixed=bin2bin_nuisance_fixed,
            ),
            bkg_shape_alphan=dict(
                ...
                uncertainty={
                    "yaml": f"{common_input_location}/files/bkg_bin2bin_v2.yaml",
                    "uncertainty": "alphan_bin2bin", "binwidth": "bin_width"
                },
                fixed=bin2bin_nuisance_fixed,
            ),
            bkg_shape_geonu=dict(
                ...
                uncertainty={
                    "yaml": f"{common_input_location}/files/bkg_bin2bin_v2.yaml",
                    "uncertainty": "geonu_bin2bin", "binwidth": "bin_width"
                },
                fixed=bin2bin_nuisance_fixed,
            ),
            bkg_shape_reactors_elbl=dict(
                ...
                uncertainty={
                    "yaml": f"{common_input_location}/files/bkg_bin2bin_v2.yaml",
                    "uncertainty": "reactors_elbl_bin2bin",
                    "binwidth": "bin_width"
                },
                fixed=bin2bin_nuisance_fixed,
            ),
            bkg_shape_lihe_tao=dict(
                ...
                uncertainty={
                    "yaml": f"{common_input_location}/files/bkg_bin2bin_tao.yaml",
                    "uncertainty": "lihe_bin2bin_tao", "binwidth": "bin_width"
                },
                fixed=bin2bin_nuisance_fixed,
            ),
            bkg_shape_fastn_tao=dict(
                bundle=dict(name="bin2bin_uncertainty", version="v01", major=()),
                name="fastn_shape_nuisance_tao",
                mode="relative",
                edges_target=tao_edges,
                uncertainty={
                    "yaml": f"{common_input_location}/files/bkg_bin2bin_tao.yaml",
                    "uncertainty": "fastn_bin2bin_tao", "binwidth": "bin_width"
                },
                fixed=bin2bin_nuisance_fixed,
            ),
            bkg_shape_ff_tao=dict(
                bundle=dict(name="bin2bin_uncertainty", version="v01", major=()),
                name="ff_shape_nuisance_tao",
                mode="relative",
                edges_target=tao_edges,
                uncertainty={
                    "yaml": f"{common_input_location}/files/bkg_bin2bin_tao.yaml",
                    "uncertainty": "ff_bin2bin_tao", "binwidth": "bin_width"
                },
                fixed=bin2bin_nuisance_fixed,
            ),
        )
        ...
        if self.opts.worst_case_fit_pars:
            print(Fore.MAGENTA+"Worst case fitting mode: fix osc pars, release baseline_wc and pmns_io"+Style.RESET_ALL)
        ...
        if self.opts.combination:
            print(Fore.MAGENTA+"Enable combination mode"+Style.RESET_ALL)
            self.cfg.reactor_spectrum_shape_unc.fixed = True

        if self.opts.comparison:
            print(Fore.MAGENTA+"Using linear interpolation of Eres for the consistency mode"+Style.RESET_ALL)
            self.cfg.eres_sigmarel_tao.preinterpolate = False

        if "inputs" in self.opts.old:
            print(Fore.MAGENTA+"Old input numbers"+Style.RESET_ALL)
            self.cfg.tao_numbers.pars = f"{common_input_location}/files/tao_input_numbers.py"

        if "bkg" in self.opts.old:
            print(Fore.MAGENTA+"Old background Acc/Li spectra"+Style.RESET_ALL)
            self.cfg.bkg_spectra_tao = dict(
                bundle=dict(name="root_histograms", version="v05"),
                filename=f"{common_input_location}/inputs_ihep/TAOoutfromBackgroundPSD_6year.root",
                formats=["histBkgShape_Acc", "histBkgShape_Li9"],
                names=["acc_spectrum_tao", "lihe_spectrum_tao"],
                labels=["Accidentals TAO|(norm spectrum)", "9Li/8He TAO|(norm spectrum)"],
                normalize=True,
            )

        if "eres" in self.opts.old:
            print(Fore.MAGENTA+"Old energy resolution"+Style.RESET_ALL)
            self.cfg.eres_smearing_tao=dict(
        ...
        if "eresimpl" in self.opts.old:
            print(Fore.MAGENTA+"Old energy resolution implementation (bin center)"+Style.RESET_ALL)
            self.cfg.eres_smearing['mode'] = 'bincenter'
            self.cfg.eres_smearing_tao['mode'] = 'bincenter'

        if "eleak" in self.opts.old:
            print(Fore.MAGENTA+"Old energy leakage matrix"+Style.RESET_ALL)
            self.cfg.eleak_tao=dict(
                bundle=dict(name="detector_energy_leakage_root", version="v02", names=lambda s: s + "_tao"),
                filename=f"{common_input_location}/{common_input_root_file}",
    ...
    def register(self):
        ...
        futurens[("juno", "variance_all")] = outputs.variance_juno_all.juno.subst
        futurens[("juno", "variance_bkg")] = outputs.variance_juno_bkg.juno.subst
        futurens[("juno", "variance_stat")] = outputs.ibd_juno_all.juno.subst
        futurens[("juno", "variance_ibd")] = outputs.variance_juno_ibd.juno.subst
