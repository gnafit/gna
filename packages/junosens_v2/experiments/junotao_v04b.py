...
class exp(baseexp):
    """\
JUNO+TAO experiment implementation v04b

Presented in a comparison on 22.02.2021

Changes since junotao_v04:
    - Do not use 0.95 scale correction
    - Add interpolated bump correction
    - Switch to the Strumia-Vissani cross section by default
    - Parameters update:
      * HZ scale set to 0.5 (valid for 6 years)
      * Incorporate duty_cycle into the livetime - to be reverted
      * Update bkg rates: new geo-ν rate (1.2)
      * Scale backgrounds with the livetime, not efflivetime
...
"""
    ...
    def initparser(cls, parser, namespace):
        ...
        parser.add_argument('--xsec', choices=['vogel', 'strumia'], default='strumia', help='IBD cross section')
    ...
    def init_formula(self):
        ...
        self.formula = [
                ...
                'baseline_tao_m',
                'baseline[d,r]',
                'livetime=bracket(daq_years*seconds_in_year*duty_cycle)',
                'livetime_tao=bracket(daq_years_tao*seconds_in_year*duty_cycle_tao)',
                'conversion_factor',
                ...
                '''anuspec_rd_full = reactor_active_norm
                                        *( sum[i]| power_livetime_factor
                                        ...
                                        + snf_in_reac''',
                'bump_correction_base| bump_correction_coarse_centers(bump_correction_coarse()), enu()',
                'bump_correction| bump_correction_coarse()',
                '''cross_section = bracket(
                                    ibd_xsec(enu(), ctheta_mesh())*
                ...
                'cmp_juno_case4  = cmp_juno_case3+cmp_bkg_juno',
                ]
    ...
    def init_configuration(self):
        ...
        common_input_root_file='deprecated/JUNOInputs2021_02_06.root'
        self.cfg = NestedDict(
                #
                # Numbers
                #
                ...
                numbers = dict(
                    ...
                    pars = f'{common_input_location}/deprecated/files/juno_input_numbers.py',
                    skip = ('percent',),
                    state= 'fixed'
                    ),
                bkg_shape_unc = dict(
                ...
                bkg_rate = dict(
                        bundle = dict(name="parameters", version = "v06"),
                        pars = f'{common_input_location}/deprecated/files/bkg_rates_v2.yaml',
                ...
                ibd_xsec_vogel = dict(
                    bundle = dict(name='xsec_ibd', version='v03', inactive=self.opts.xsec!='vogel'),
                    order = 1,
                    ),
                ibd_xsec_strumia = dict(
                    bundle = dict(name='xsec_ibd_ext', version='v04', inactive=self.opts.xsec!='strumia'),
                    order = 1,
                    filename = f'{common_input_location}/{common_input_root_file}',
                    objectname = 'dsigma_dcos_Enu_cos',
                    verbose = True
                    ),
                oscprob = dict(
