...
class exp(baseexp):
    """\
JUNO+TAO experiment implerentation v04l

To be presented on 2021-05-24

- Temporary changes to be fixed
    + Comment TAO until the enu_matrix/enu_subst are correctly dispatched: see junotao_v05a
    + Disable spectral uncertainties

Changes since junotao_v04j (not k):
    + Switch to the testing input file [JUNOInputs2021_05_20.root]:
        * Updated LSNL curve
        * Updated Li/He shape (due to LSNL update)
        * Updated Acc spectrum shape
    + Updated background rates [bkg_rates_v4.yaml]:
        * [v4] [2021-05-19] Acc rate 0.8->0.6, Li/He rate 1.6->1.2 due to muon efficiency update.
    + Update energy scale:
        * LSNL (see above)
        * scaled eres statistical/noisce terms [juno_eres_v2.yaml]
    + Updates baselines according new measurement. Reference: PPNP paper in DocDB-6387 [reactor_baselines_v2.yaml]
