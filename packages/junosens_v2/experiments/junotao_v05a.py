...
class exp(baseexp):
    """\
JUNO+TAO experiment implementation v05a

Based on version v04j and v04l (new inputs)

Updates:
    + Reimplement SNF and offequilibrium with general bundles
    + Introduce new index mode: subst, matrix. Compute enu, cross section, etc for 2 neutrino energies:
        - subst goes to JUNO
        - matrix goes to TAO
        - there is a backup matrix version of JUNO as well
    + Uncomment TAO
    + [WIP] Enable spectral uncertainties
    + SNF and Offequilibrium:
        - enable extrapolation below 1.805
        - add a commandline option --consistent-snf-offeq-extrap for the cross check with previous [to be removed]

Derived from:
    - Combined:
        + [2021.05.26] merged in junotao_v04l
        + [2021.05.04] junotao_v04j
    - [2021.04.13] junotao_v04h
    - [2021.03.30] junotao_v04g
    - [2021.03.25] junotao_v04f
...
"""
    ...
    def initparser(cls, parser, namespace):
        ...
        # Consistency flags
        parser.add_argument('--consistent-snf-offeq-extrap', action='store_true') # TODO: deprecate
    ...
    def init_formula(self):
        ...
        self.formula = [
                ...
                'power_livetime_factor_nominal = conversion_factor * thermal_power_correction[r] * thermal_power_nominal[r] * fission_fractions_nominal[r,i]() / energy_per_fission_avg_nominal',
                ...
                'snf_in_reac = snf_norm * snf_individual_scale[r] * nominal_spec_per_reac() * snf_fine()',
                #
                # Offequilibrium
                #
                ...
                '''ibd_juno_all=norm_reac*norm_juno*anue_spectrum_scale_final()* rebin|
                         {eres} {lsnlmatrix}
                           rebin_juno_internal |
                               kinint2_juno(
                                   DistortSpectrum(
                ...
                # Total observation
                #
                'observation_juno_all=ibd_juno_all + bkg_juno',
                # 'variance_juno_all = staterr2(observation_juno_all) + bkg_shape_variance',
                #
                # TAO part
                #
    ...
    def init_configuration(self):
        ...
        common_input_root_file='deprecated/JUNOInputs2021_05_20.root'
        self.cfg = NestedDict(
                #
                # Numbers
                #
                ...
                bkg_rate = dict(
                        bundle = dict(name="parameters", version = "v06"),
                        pars = f'{common_input_location}/files/bkg_rates_v4.yaml',
                ...
                snf_individual_scale =  dict(
                        bundle = dict(name="parameters", version = "v06", major=''),
                        labels={
                            'snf_individual_scale': 'SNF scale for {autoindex}',
                            },
                        pars =  {
                            'snf_individual_scale': (1.0, 100),
                            'uncertainty_mode': 'percent',
                            },
                        ),
                snf_norm = dict(
                ...
                # Energy resolution
                eres = dict(
                        bundle = dict(name='detector_eres_normal', version='v02', major=''),
                ...
                eres_pars = dict(
                        ...
                        pars = f'{common_input_location}/files/juno_eres_v2.yaml',
                        depth = 2
                    ),
                #
                # Backgrounds
                ...
                bkg_shape_fastn = dict(
                        ...
                        edges_target = juno_edges,
                        uncertainty={
                            'yaml': f'{common_input_location}/files/bkg_bin2bin.yaml',
        ...
        if self.opts.consistent_snf_offeq_extrap:
            print('\033[31mWarning! Using consistency mode: SNF extrapolation is turned off\033[0m')
            self.cfg['snf_fine.strategy'] = ('constant', 'constant')
            self.cfg['offeq_fine.strategy'] = ('constant', 'constant')
    ...
    def register(self):
        ...
        futurens[('juno', 'initial')]  = outputs.rebin_juno_internal.juno
        try:
        ...
        futurens[('juno', 'final')]        = outputs.observation_juno_all.juno.subst
        futurens[('juno', 'final_matrix')] = outputs.observation_juno_all.juno.matrix
