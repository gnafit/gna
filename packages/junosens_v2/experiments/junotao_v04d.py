...
Presented on 2021.03.08

Changes since junotao_v04c:
    - Switch to the own Vogel+Beacom cross section
    - Update to new bkg rates (v3, fixed acc rate)
    - Change internal binning to 5 keV
    - Add reference matter oscprob
    - Unify oscillation probability parameters

Derived from:
    - [2021.03.02] junotao_v04b
...
"""
    ...
    def initparser(cls, parser, namespace):
        ...
        # osc prob
        parser.add_argument('--oscprob', choices=['vacuum', 'matter-approx', 'matter'], default='matter-approx', help='oscillation probability type')
        parser.add_argument('--xsec', choices=['vogel', 'dyb'], default='vogel', help='IBD cross section')
    ...
    def init_configuration(self):
        ...
        common_input_root_file='deprecated/JUNOInputs2021_03_05.root'
        self.cfg = NestedDict(
                #
                # Numbers
                #
                ...
                ibd_xsec_vogel = dict(
                    bundle = dict(name='xsec_ibd', version='v04', inactive=self.opts.xsec!='vogel'),
                    order = 1,
                    verbose = 2,
                    constants = {
                        'PhaseFactor': 1.71465,
                ...
                ibd_xsec_dyb = dict(
                    bundle = dict(name='xsec_ibd_ext', version='v04', inactive=self.opts.xsec!='dyb'),
                    order = 1,
                    filename = f'{common_input_location}/{common_input_root_file}',
                    objectname = 'dsigma_dcos_Enu_cos_DYB',
                    verbose = True
                    ),
                oscpars = dict(
