...
class exp(baseexp):
    """\
JUNO+TAO experiment implementation v04k

A verstion to test internal binning
Updates:

- Temporary changes to be fixed
    + Comment TAO until the enu_matrix/enu_subst are correctly dispatched
    + Disable spectral uncertainties

Changes since junotao_v04j:

Derived from:
    - [2021.04.30] junotao_v04j
...
"""
    ...
    def initparser(cls, parser, namespace):
        ...
        binning.add_argument('--step-integration', type=float, help='Integration binning step, keV')
        binning.add_argument('--step-matrix', type=float, help='Matrix effect step step, keV')
        binning.add_argument('--step-divider', type=int, help='Divider for the binning around the threshold')
    ...
    def init_formula(self):
        ...
        self.formula = [
                ...
                'anuspec_unc_corrected=anuspec[i](enu_juno())',
                #
                # SNF
                #
                ...
                '''ibd=norm_reac*norm_juno*anue_spectrum_scale_final()* rebin|
                         {eres} {lsnlmatrix}
                           rebin_juno_internal |
                               kinint2_juno(
                                   DistortSpectrum|
    ...
    def init_configuration(self):
        step_juno_divider     = 4
        step_juno_integration = 0.005
        step_juno_matrix      = 0.005
        assert (step_juno_matrix%step_juno_integration)==0
        if self.opts.step_matrix:
            step_juno_matrix = self.opts.step_matrix*1.e-3
        if self.opts.step_integration:
            step_juno_integration = self.opts.step_integration*1.e-3
        if self.opts.step_divider:
            step_juno_divider = self.opts.step_divider
        print(f'JUNO binning step: int {step_juno_integration*1000} keV (thresh 1/{step_juno_divider}), mat {step_juno_matrix*1000}')
        step_tao_matrix = 0.01
        edges_internal = np.arange(0.0, 12.001, step_juno_matrix)
        edges_internal_tao = np.arange(0.0, 12.001, step_tao_matrix)
        ...
        common_input_root_file='deprecated/JUNOInputs2021_03_22.root'
        self.cfg = NestedDict(
                #
                # Numbers
                #
                ...
                kinint2 = dict(
                    ...
                    xedgescfg  = [
                                   (0.0, step_juno_integration,   0),
                                   (0.8, step_juno_integration/step_juno_divider, 3),
                                   (1.1, step_juno_integration,   3),
                                   (12, None, None),
                                ],
                    yorder    = 5,
                    instances = {
    ...
    def register(self):
        ...
        futurens[('juno', 'data_in')]  = inputs.rebin_data.juno['00']
        futurens[('juno', 'data')]     = outputs.rebin_data.juno
