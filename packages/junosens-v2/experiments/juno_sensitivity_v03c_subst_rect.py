...
class exp(baseexp):
    """
JUNO experiment implementation v03c (binning, HistNonlinearityB, rectangular integration)

This is a clone of v03b_subst used to study the binning effect with rectangular integrator, reported in #132
...
Derived from:
    ...
    - [2020.11] juno_sensitivity_v03b_subst

Changes since previous implementation [juno_sensitivity_v03b_subst]:
    - configurable integrator mode
"""
    ...
    def initparser(cls, parser, namespace):
        ...
        #Integrator
        parser.add_argument('--integrator', choices=['GL', 'rect', 'rect_left', 'rect_right'], default='GL', help='Choose integrator mode')
    ...
    def init_configuration(self):
        ...
        self.cfg = NestedDict(
                ...
                # General
                kinint2 = OrderedDict(
                    bundle    = dict(name='integral_2d1d', version='v05', names=dict(integral='kinint2')),
                    variables = ('evis', 'ctheta'),
                    edges     = edges,
                    #  edges   = np.linspace(0.0, 12.001, 601),
                    xorders   = 4,
                    yorder    = 5,
                    mode      = self.opts.integrator
                    ),
                rebin = OrderedDict(
