...
{
    switch(gState) {
      case NotInitialized : so << "NotInitialized"; break;
      case InitializedOnly : so << "InitializedOnly"; break;
      case OnHost : so << "OnHost"; break;
      case OnDevice : so << "OnDevice"; break;
      case Crashed : so << "Crashed"; break;
      default : so.setstate(std::ios_base::failbit);
    }
    return so;
}
