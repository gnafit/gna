...
__global__ void fullProb(T DMSq12, T DMSq13, T DMSq23, T weight12,
	...
	devCompCP[x] = 0.0;
	// TODO: add sharing
	T halfSin12, halfSin13, halfSin23, halfCos12, halfCos13, halfCos23;
	...
	ret[x] = 2.0 * (weight12 * devComp12[x] + weight13 * devComp13[x] +
...
template <typename T>
void calcCuFullProb(GNAcuOscProbMem<T>& mem, T DMSq12, T DMSq13, T DMSq23,
		    T weight12, T weight13, T weight23, T weightCP, T* ret, T L,
		    T* Enu, int EnuSize, bool sameAB) {
	...
	for (int i = 0; i < streamcount; i++) {
		...
		int cpySize;
		if (i != streamcount - 1)
			cpySize = EnuSize * sizeof(T);
		else {
			cpySize = (fullEnuSize - i * EnuSize) * sizeof(T);
		}
		err = cudaMemcpyAsync((void**)&mem.devEnu[k], (void**)&Enu[k],
				      cpySize, cudaMemcpyHostToDevice,
				      workerstreams[i]);
		if (err != cudaSuccess) {
			printf(
			    "ERROR: unable to copy memory from host to device "
			    "in for! \n");
	...
	err = cudaMemcpyAsync(ret, mem.devRet, alloc_size,
...
void calcCuFullProb_double(GNAcuOscProbMem<double>& mem, double DMSq12,
			   double DMSq13, double DMSq23, double weight12,
			   double weight13, double weight23, double weightCP,
			   double* ret, double L, double* Enu, int EnuSize,
			   bool sameAB) {
	calcCuFullProb<double>(mem, DMSq12, DMSq13, DMSq23, weight12, weight13,
			       weight23, weightCP, ret, L, Enu, EnuSize,
			       sameAB);
}

void calcCuFullProb_float(GNAcuOscProbMem<float>& mem, float DMSq12,
			  float DMSq13, float DMSq23, float weight12,
			  float weight13, float weight23, float weightCP,
			  float* ret, float L, float* Enu, int EnuSize,
			  bool sameAB) {
	calcCuFullProb<float>(mem, DMSq12, DMSq13, DMSq23, weight12, weight13,
			      weight23, weightCP, ret, L, Enu, EnuSize, sameAB);
}
