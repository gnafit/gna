...
__device__
double vectorsum(double** array, int start, int limit){
	double del = 0;
...
__global__
void normalize(double** array, double** ans_array, int* n) { //NOTE: Normalize and normalize_segment can lead to cudaErrorLaunchTimeout on big amount of elements due to watchdog!
...
__global__
void normalize_segment(double** array, double** ans_array, int* n, int start, int limit) {
	int Col = blockIdx.x*blockDim.x+threadIdx.x;
	if (Col < n[0])
...
//CUDA FUNCTION INVOKATION
	...
	normalize<<<GridSize(M), CU_BLOCK_SIZE>>>(dev_ptr_array, dev_ans_ptr_array, dev_dim1_array);
