...
__global__
void weightedsum(double** array, double** ans_array, double* weights, int n, int m) {
...
__global__
void weightedsumfill(double** array, double** ans_array, double* weights, double k, int n, int m) {
	int x = blockDim.x * blockIdx.x + threadIdx.x;
	if (x >= m) return;
...
//Weights allocation
	...
	double *weights_array = new double[N];
	double *dev_weights_array;
	std::cout<<"WEIGHTS ARE:"<<std::endl;
	for (int i = 0; i < N; i++){
		weights_array[i] = N - i;
		std::cout<<weights_array[i]<<" ";
	}
	std::cout<<std::endl;
	cudaMalloc(&dev_weights_array, N * sizeof(double));
	err =
		cudaMemcpy(dev_weights_array, weights_array, N * sizeof(double), cudaMemcpyHostToDevice);	
	if (err != cudaSuccess) {
...
//CUDA FUNCTION INVOKATION
	...
	double k = 7.5;
	weightedsumfill<<<GridSize(M), CU_BLOCK_SIZE>>>(dev_ptr_array, dev_ans_ptr_array, dev_weights_array, k, N, M);
//CUDA FUNCTION END
