...
EnergyResolutionWithSparse::EnergyResolutionWithSparse() {
  ...
  transformation_(this, "smear")
      ...
      .output("Nrec")
      .types(Atypes::pass<0>,
         [](EnergyResolutionWithSparse *obj, Atypes args, Rtypes /*rets*/) {
           obj->m_datatype = args[0];
           obj->fillCache();
         })
       .func(&EnergyResolutionWithSparse::calcSmear);
}

double EnergyResolutionWithSparse::relativeSigma(double Etrue) const noexcept{
  return sqrt(pow(m_a, 2)+ pow(m_b, 2)/Etrue + pow(m_c/Etrue, 2));
}

double EnergyResolutionWithSparse::resolution(double Etrue, double Erec) const noexcept {
...
void EnergyResolutionWithSparse::fillCache() {
...
void EnergyResolutionWithSparse::calcSmear(Args args, Rets rets) {
  ...
  std::transform(events_true, events_true + insize, &events_true_sanitized[0],
                [](auto event){return (!std::isnan(event) ? event : 0.);});


  /* std::cout << "Time for usual stuff " << dur1.count() << std::endl; */

  Eigen::Map<Eigen::VectorXd> mapped(events_true_sanitized, outsize);
  rets[0].x = m_sparse_cache * mapped; 
