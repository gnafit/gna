...
-- contour --chi2 /tmp/out_C14.hdf5  --minimizer min --plot chi2minplt \
   --no-shift --xlog --xlabel "\$ \rho_{rel} \$" --ylabel "\$ \Delta \chi^2 \$" \
   --title "Coincidence window is 300 ns" \
   --show
   # --output chi2_C14.pdf
#--pullminimizer min
...
# -- scan  --lingrid common.rho_C14 1e-19 1e-16 40 --minimizer min \
   # --verbose --output /tmp/out_C14.hdf5 \
