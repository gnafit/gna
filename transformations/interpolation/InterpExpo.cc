...
double InterpExpo::interpolation_formula_below(double x, double y, double k, double point) const noexcept {
      return y * std::exp((x - point)*k);
}

double InterpExpo::interpolation_formula_above(double x, double y, double k, double point) const noexcept {
      return y * std::exp((x - point)*k);
}
