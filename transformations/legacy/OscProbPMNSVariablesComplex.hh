...
class OscProbPMNSVariablesComplex: public ParametersGroup {
public:
  static const size_t Nnu = 3;

  OscProbPMNSVariablesComplex(GNAObject *parent, Neutrino from, Neutrino to, const std::vector<std::string>& names={})
    : ParametersGroup(parent, fields(from, to, names))
    { }
  OscProbPMNSVariablesComplex(GNAObject *parent, std::vector<std::string> params, Neutrino from, Neutrino to, const std::vector<std::string>& names={})
    : OscProbPMNSVariablesComplex(parent, from, to, names)
    { initFields(params); }
...
public:
  OscProbPMNSExpressions(Neutrino from, Neutrino to, const std::vector<std::string>& names={})
    : ExpressionsProvider(new OscProbPMNSVariablesComplex(this, from, to, names))
    { }
};
