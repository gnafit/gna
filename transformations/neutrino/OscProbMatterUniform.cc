...
#include "TypesFunctions.hh"
    ...
    GNA::GNAObjectTemplates::OscProbMatterT<FloatType>::OscProbMatterT(Neutrino from, Neutrino to)
        : OscProbPMNSBaseT<FloatType>(from, to), m_from(from), m_to(to)
    {
        ...
        this->variable_(&m_rho, "rho"); // g/cm3
        this->transformation_("oscprob")
            ...
            .func(&OscProbMatterT::calcOscProb);
    
        this->m_param->variable_("DeltaMSq12");
        this->m_param->variable_("DeltaMSq13");
        this->m_param->variable_("DeltaMSq23");
        this->m_param->variable_("Theta12");
        this->m_param->variable_("Theta13");
        this->m_param->variable_("Theta23");
        this->m_param->variable_("Alpha");
        this->m_param->variable_("Delta");
    
    }
    
    template<typename FloatType>
    void GNA::GNAObjectTemplates::OscProbMatterT<FloatType>::calcOscProb(FunctionArgs fargs) {
      ...
      double SinSq12 = pow(sin(this->m_param->Theta12), 2);
      double Sin12 = sin(this->m_param->Theta12);
      double Cos12 = cos(this->m_param->Theta12);
      double CosSq12 = 1-SinSq12;
      double SinSq23 = pow(sin(this->m_param->Theta23), 2);
      double Sin23 = sin(this->m_param->Theta23);
      double Cos23 = cos(this->m_param->Theta23);
      double CosSq23 = 1-SinSq23;
      double SinSq13 = pow(sin(this->m_param->Theta13), 2);
      double Sin13 = sin(this->m_param->Theta13);
      double Cos13 = cos(this->m_param->Theta23);
      ...
      double d1 = (-this->m_param->DeltaMSq12.value() - this->m_param->Alpha.value()*this->m_param->DeltaMSq13.value())/3;
      double d2 = ( this->m_param->DeltaMSq12.value() - this->m_param->Alpha.value()*this->m_param->DeltaMSq23.value())/3;
      double d3 = this->m_param->Alpha.value()*(this->m_param->DeltaMSq13.value() + this->m_param->DeltaMSq23.value())/3;
    
      double deltacp;
      if (m_from.kind == 1) {
        deltacp = this->m_param->Delta.value();
      } else if (m_from.kind == -1) {
        deltacp = -1 * this->i_param->Delta.value();
      } else {
      ...
      if (m_from.flavor == m_to.flavor) {
        if (m_from.flavor == Neutrino::Flavor::Muon) {
          Eigen::ArrayXd pconst = xi1_0.square() + xi1_1.square() + xi1_2.square();
          auto p01 = 2*((1.0L/2.0L)*L*(E0 - Em)/E).cos()*(xi1_0*xi1_1).abs();
          auto p02 = 2*((1.0L/2.0L)*L*(Em - Ep)/E).cos()*(xi1_0*xi1_2).abs();
        ...
        if (m_from.flavor == Neutrino::Flavor::Electron) {
          ...
          auto p01 = 2*((1.0L/2.0L)*L*(E0 - Em)/E).cos()*(xi0_0*xi0_1).abs();
          auto p02 = 2*((1.0L/2.0L)*L*(Em - Ep)/E).cos()*(xi0_0*xi0_2).abs();
