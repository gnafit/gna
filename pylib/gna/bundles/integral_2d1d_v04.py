...
class integral_2d1d_v03(TransformationBundle):
    ...
    def check_cfg(self):
        ...
        if not self.cfg.mode:
            ...
            raise Exception('Invalid integrator mode')
        else:
    ...
    def build(self):
        if self.xorders.size>1:
            ...
            if self.mode == 'GL':
                ...
                self.integrator = R.Integrator2Rect(self.edges.size-1, int(self.xorders[0]), self.edges, self.yedges.size-1, self.cfg.yorder, self.yedges)
