...
mod_proj_orig  = fcn_inverse(orig)

mod       = np.linspace(orig[0], orig[-1], (orig.size-1)*2+1)
mod_proj  = fcn_inverse(mod)
...
print('Edges modified projected back:', mod_proj_orig.shape, mod_proj_orig)

Orig = C.Points(orig)
Orig_proj = C.Points(orig_proj)
# Mod = C.Points(mod)
Mod_proj_orig = C.Points(mod_proj_orig)
Mod_proj = C.Points(mod_proj)
ntrue = C.Histogram(orig, np.ones(orig.size-1))
nmod = C.Histogram(mod, np.ones(mod.size-1))

nlB = C.HistNonlinearityB(R.GNA.DataPropagation.Propagate)
nlB.set(ntrue.hist, Orig_proj, Mod_proj_orig)
nlB.add_input()

nlC = C.HistNonlinearityC(R.GNA.DataPropagation.Propagate)
nlC.set(ntrue.hist, Orig_proj, Mod_proj_orig)
nlC.add_input()

nlD = C.HistNonlinearityD(R.GNA.DataPropagation.Propagate)
nlD.set(ntrue.hist, Orig_proj, nmod.hist, Mod_proj)
nlD.add_input()

nlB.printtransformations()
nlC.printtransformations()
nlD.printtransformations()

print('Get data')
mat_b = nlB.matrix.FakeMatrix.data()
mat_c = nlC.matrix.FakeMatrix.data()
mat_d = nlD.matrix.FakeMatrix.data()
...
print( 'Mat D and its sum' )
print( mat_d.shape )
print( mat_d )
matbsum_d = mat_d.sum( axis=0 )
print( matbsum_d )
...
def plot_curve():
    ...
    c = ax.plot(mod_proj_orig, orig, 'o', label='Inverse')[0].get_color()
...
plot_matrix(mat_d, 'Matrix D', edges_a=orig, edges_b=mod)
