...
-- ns --value peak1.Mu 200 --value peak1.BackgroundRate 200  \
      --value peak2.Mu 200 --value peak2.BackgroundRate 200  \
 \
...
-- minimizer empty_minimizer_chi     minuit chi2_asimov1 \
 \
-- scan --lingrid peak1.Mu 150 250 1000   --minimizer empty_minimizer_chi      --verbose --output $(pwd)/tests/chi2_vs_poisson/gaussian_model/tmp/scan_chi.hdf5 \
-- scan --lingrid peak1.Mu 150 250 1000   --minimizer empty_minimizer_pois     --verbose --output $(pwd)/tests/chi2_vs_poisson/gaussian_model/tmp/scan_pois.hdf5 \
...
-- contour --chi2 $(pwd)/tests/chi2_vs_poisson/gaussian_model/tmp/scan_chi.hdf5     --minimizer chi_min1 --plot  chi2minplt --labels "poisson asdata"   --legend upper right \
	 --output $(pwd)/tests/chi2_vs_poisson/gaussian_model/img/middle_den_poisson_compare.png \
#-- spectrum --plot peak1/spectrum --savefig $(pwd)/tests/chi2_vs_poisson/gaussian_model/img/poisson_middle_spectrum.png
