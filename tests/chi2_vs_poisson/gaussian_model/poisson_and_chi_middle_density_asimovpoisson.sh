...
-- ns --value peak1.Mu 200 --value peak1.BackgroundRate 200  \
      ...
      --value peak3.Mu 200 --value peak3.BackgroundRate 200  \
 \
...
-- minimizer empty_minimizer_pois     minuit pois_asimov \
 \
-- scan --lingrid peak1.Mu 150 250 100   --minimizer empty_minimizer_chi      --verbose --output $(pwd)/tests/chi2_vs_poisson/gaussian_model/tmp/scan_chi.hdf5 \
-- scan	--lingrid peak3.Mu 150 250 100   --minimizer empty_minimizer_pois     --verbose --output $(pwd)/tests/chi2_vs_poisson/gaussian_model/tmp/scan_pois.hdf5 \
...
-- contour --chi2 $(pwd)/tests/chi2_vs_poisson/gaussian_model/tmp/scan_pois.hdf5   --minimizer pois_min --plot  chi2minplt --labels "poisson"  --legend upper right \
	 --output $(pwd)/tests/chi2_vs_poisson/gaussian_model/img/middle_den_chi_and_poiss_aspois.png
